...
IF OBJECT_ID ('DB_TEMP..TMP_MONITOR_REMOVER_OUTLIER') IS NOT NULL
	DROP TABLE DB_TEMP..TMP_MONITOR_REMOVER_OUTLIER

IF OBJECT_ID ('DB_TEMP..TMP_MONITOR_TEMPO_MEDIO_PACOTES') IS NOT NULL
	DROP TABLE DB_TEMP..TMP_CONTROLE_TEMPO_MEDIO_PACOTES
...
INTO DB_TEMP..TMP_MONITOR_REMOVER_OUTLIER
FROM TEMPO_MEDIO
...
INTO DB_TEMP..TMP_MONITOR_TEMPO_MEDIO_PACOTES
FROM BUICTRDBS.DBO.AUDIT_LOGEXECUCAOPACOTE A (NOLOCK)
	JOIN DB_TEMP..TMP_MONITOR_REMOVER_OUTLIER B
...
/*LISTA DE PACOTES NÃO EXECUTADOS, UTILIZA O TEMPO MÉDIO DOS PACOTES, REMOVENDO OUTLIERS*/
SELECT A.DESCRICAOPACOTE,
A.TEMPO_MEDIO_MIN, 
A.TEMPO_MEDIO_HOR,
A.DAT_ULT_EXECUCAO,
B.STATUSPACOTE STATUS_ATUAL, 
...
FROM #TMP_TEMPO_MEDIO_PACOTES A
	LEFT JOIN BUICTRDBS.DBO.AUDIT_LOGEXECUCAOPACOTE B (NOLOCK)
		ON A.DESCRICAOPACOTE = B.DESCRICAOPACOTE
WHERE CONVERT(VARCHAR(8),B.HORARIOINICIO,112) = CONVERT(VARCHAR(8),GETDATE(),112)
ORDER BY A.DESCRICAOPACOTE
